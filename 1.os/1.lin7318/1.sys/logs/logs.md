...
Команды для быстрого доступа к содержимому файлов.

##### A. `cat`, `head`, `tail` (Статичный просмотр)

Подходят для просмотра содержимого файлов целиком или их частей.
...
**Частая команда.**"Cледит" за файлом и выводит новые строки по мере их появления (как команда `watch`).
...
##### C. `less` (Интерактивный просмотр больших файлов)
 ...
 Подходит для просмотра больших файлов, так как загружает их постранично и позволяет осуществлять навигацию и поиск.
...
Поиск конкретных ключевых слов, ошибок или IP-адресов в логах.
...
Linux автоматом архивирует старые логи с помощью `logrotate`, добавляя расширение `.gz`.

- **Неправильно:** `cat /var/log/syslog.1.gz | gunzip`
- **Правильно (`zcat`, `zgrep`, `zless`):**
