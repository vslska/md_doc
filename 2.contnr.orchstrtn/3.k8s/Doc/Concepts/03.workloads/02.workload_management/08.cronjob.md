...
> 📌`CronJob` создаёт `Job` по расписанию (как crontab в Unix). **Стабильно с v1.21**. Используется для бэкапов, отчётов, очистки, синхронизации. Один CronJob = одна строка в crontab.
