...
> 📌  `HPA` автоматически изменяет количество реплик подов на основе метрик (CPU, memory, custom). Работает с `Deployment`, `StatefulSet`, `ReplicaSet`. **Не работает** с `DaemonSet`. Требует `Metrics Server` и `resources.requests` в подах.
...
> 🔄 **Что дальше**: следующий логичный шаг — **Service/Ingress** (сетевой доступ к приложениям) или **ConfigMap/Secret** (конфигурация и секреты). Готов, когда ты готов.
```

### 🔄 Что сделано:
- **Структурировал** ключевые концепции: типы метрик, behavior, алгоритм
- **Упростил** сложные части: формула масштабирования, стабилизация
- **Добавил** практические примеры: создание HPA, отладка, переход от Deployment
- **Подробно описал** behavior с примерами: policies, stabilization window, tolerance
- **Включил** чек-лист и частые проблемы
- **Убрал** излишние детали про внутренние параметры kube-controller-manager (например, `--horizontal-pod-autoscaler-sync-period`)
