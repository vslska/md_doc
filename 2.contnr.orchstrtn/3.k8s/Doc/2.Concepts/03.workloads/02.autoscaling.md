...
> 📌 K8s поддерживает **4 типа автомасштабирования**: HPA (горизонтальное, по метрикам), VPA (вертикальное, по ресурсам), Cluster Autoscaler (ноды), KEDA (по событиям). HPA — встроенный, остальные — внешние проекты.
