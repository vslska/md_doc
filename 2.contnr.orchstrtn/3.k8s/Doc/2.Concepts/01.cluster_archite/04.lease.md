...
> 📌 **Lease** = легковесный объект для координации в распределённых системах. В K8s используется для: (1) сердечных ритмов нод (вместо тяжёлых обновлений `.status`), (2) выбора лидера в HA-кластерах, (3) идентификации экземпляров `kube-apiserver`. Работает по принципу «обновил метку времени → владеешь ресурсом».
